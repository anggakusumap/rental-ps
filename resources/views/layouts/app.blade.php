            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-16">
                    <div class="flex items-center">
                        ...
                        <span class="text-xl font-bold">The Room PlayStation</span>
                    </div>
                    <div class="flex items-center space-x-4">
                        <span>{{ auth()->user()->name }}</span>
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="bg-indigo-700 px-4 py-2 rounded hover:bg-indigo-800">Logout</button>
                        </form>
                    </div>
                </div>
            </div>
        ...
        <div class="flex">
            <aside class="w-64 bg-white shadow-md min-h-screen">
                <nav class="mt-5">
                    ...
                    <a href="{{ route('customers.index') }}" class="flex items-center px-6 py-3 hover:bg-gray-100 {{ request()->routeIs('customers.*') ? 'bg-indigo-50 text-indigo-600' : '' }}">
